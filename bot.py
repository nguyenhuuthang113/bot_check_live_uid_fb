...
async def checker(context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid in list(data.keys()):
        try:
            old = data.get(uid)
            # requests là sync nên chạy ở thread để không block event loop
            new = await asyncio.to_thread(check_uid, uid)
...
def main():
    ...
    app.job_queue.run_repeating(checker, interval=60, first=10)
